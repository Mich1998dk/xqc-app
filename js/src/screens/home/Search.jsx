...
import { StyleSheet, FlatList, View, Platform, TextInput, TouchableHighlight, } from "react-native";
...
export default function Search({ navigation, route }) {
    ...
    return (<Container>
      <Header title="Search" reset={mode === "terms"} onPressReset={() => {
      ...
      <View style={{ width: "100%", alignSelf: "center" }}>
        <View style={styles.inputContainer}>
          ...
          <TextInput placeholder="Search for a keyword" value={state.search} placeholderTextColor="#888" onChangeText={(e) => setState({ ...state, search: e })} style={[
            styles.input,
            Platform.OS === "web" ? webStyles : {},
        ]}/>
        </View>
        {state.currentlySearched.length > 0 && (<View>
            <Text.Button style={{ alignSelf: "center", marginBottom: 10 }}>
              Showing results for '{state.currentlySearched}'
            </Text.Button>
          </View>)}
        <ScrollView>
          {(redux.searchResults.length === 0 || state.search.length > 0) && (<FlatList data={search} style={{ height: 600 }} keyExtractor={(item) => item.toString()} renderItem={({ item, index }) => {
                return (<TouchableHighlight underlayColor={colors.box} style={styles.result} onPress={() => {
                        if (mode === "terms") {
          ...
          {redux.searchResults.length > 0 && state.search.length === 0 && (<ImageRenderer data={redux.searchResults} navigation={navigation}/>)}
        </ScrollView>
      </View>
    </Container>);
}
