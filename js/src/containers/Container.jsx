...
export default function Container({ children, isKeyBoardAvoiding, style, model, navigation, }) {
    ...
    return (<SafeAreaView style={styles.container}>
      ...
      {isKeyBoardAvoiding ? (<KeyboardAvoidingView behavior={Platform.OS == "ios" ? "padding" : "height"} style={[styles.content, style]}>
          {children}
        </KeyboardAvoidingView>) : (<View style={[styles.content, style]}>{children}</View>)}

      {redux.loading && <Loader loadingTitle="Please wait.."/>}
      {redux.search && <Search />}
      {redux.menu && (<Menu onExit={() => {
                dispatch(setMenu(false));
                dispatch(reset());
                navigation?.goBack();
            }} onClickHelp={() => {
            ...
            }} canQuickSave={model !== undefined} onClickQuickSave={() => {
                quickSaveModel();
            }}/>)}
    </SafeAreaView>);
}
