...
export default function TimePicker({ type }) {
    ...
    const render = () => {
        return DATA.map((item, index) => {
            return (<TouchableOpacity key={index} style={styles.time} onPress={() => {
                    handleChange(item);
                }}>
      ...
      <Animatable.View animation="zoomIn" duration={240}>
        <View style={styles.inner}>
          <View style={styles.top}>
            <IconButton title="AM" secondary={!state.isAm} style={{ marginRight: 10, width: 60 }} onPress={() => setState({ ...state, isAm: true })}/>
            <IconButton title="PM" style={{ width: 60 }} secondary={state.isAm} onPress={() => setState({ ...state, isAm: false })}/>
          </View>
          ...
          <View style={{
            ...
            handleChange(0);
            dispatch(setTimePicker(false));
        }} title={"Reset"} style={{ width: 100, marginRight: 10 }}/>
            <IconButton onPress={() => {
            dispatch(setTimePicker(false));
        }} title={"Close"} style={{ width: 100 }}/>
          </View>
        </View>
      </Animatable.View>
    </Animatable.View>);
