...
import { StyleSheet, TouchableOpacity, View, Platform, TextInput, FlatList, TouchableHighlight, } from "react-native";
    ...
    return (<Animatable.View style={styles.container} animation="fadeIn" duration={240}>
      <Animatable.View animation="zoomIn" duration={240}>
        <View style={{
          ...
          <TouchableOpacity onPress={() => dispatch(setSearch(false))} style={{
            alignSelf: "center",
            alignItems: "center",
            marginLeft: 10,
        }} activeOpacity={0.7}>
        ...
        <FlatList data={state.search.length >= 2
            ? terms
                .filter((item) => item.includes(state.search.toLowerCase()))
                .slice(0, 50)
            : []} style={{ height: 300 }} keyExtractor={(item) => item.toString()} renderItem={({ item, index }) => {
